...
// ── FolkFriend detector parameters ────────────────────────────────────────────
// The WASM engine is a generic detector: its own defaults are upstream
// FolkFriend's, and the values that suit Cadence are set HERE, by the caller
// (ffWorker.ts at init; experiments/noise-study reads the same constants so the
// test fixtures are generated with exactly what the app runs).
//
// CURRENT OPTIMUM, from the 2026-09-13 ranking study
// (experiments/ranking-study/CAMPAIGN_V3.md). Together with
// absentObservationRatio in detectionTemporalConfig.ts, on 7 annotated sessions
// (341 tunes), with windows regenerated by the production engine itself: 318
// found, 12 false positives, mean IoU 88.0 %, 90th-percentile boundary errors
// 16 s / 15 s — against 311, 20, 79.5 %, 25 s / 43 s before. Chosen on six
// sessions and scored on the seventh (study harness), the same choices gave
// 318-320 found, 12.5-13.5 false positives, IoU 88.2 %. Re-run that study
// (and its held-out validation) before moving any of these.

/** Tempo search range in crotchets per minute (upper bound exclusive, step 5).
 *  Engine default 60-240. FolkFriend's tempo selector measures rounding error in
 *  FRAMES, so a faster candidate tempo (a finer grid) always fits better: it
 *  overshoots. Same audio, when allowed to go faster the right tune's rank got
 *  worse 97 times for 20 better (p ≈ 1e-13). Capping at 180 gained 70 net rank-1
 *  windows. Widening to 320 was measured too: no effect. */
export const FF_TEMPO_MIN_BPM = 60;
export const FF_TEMPO_MAX_BPM = 180;

/** Settings re-aligned by Needleman-Wunsch after the n-gram pass. Engine
 *  default 2000. A tune with many settings is reported with its BEST one, so
 *  aligning more settings raises the right tune's score: +58 net rank-1 windows
 *  for 7 lost, no effect on noise, query ×1.5 (scalar build). 8000 gains 2 more
 *  tunes for 1 more false positive at ×2.4 — not worth it yet. Lowering it loses
 *  real detections (see ff_config.rs). */
export const FF_QUERY_REPASS_SIZE = 4000;

/** Contours shorter than this many symbols (quavers) are not queried. Engine
 *  default 0. Scores are normalised by the shorter sequence, so a very short
 *  contour finds a chance alignment somewhere in 55 000 settings; a slower tempo
 *  cap makes noise transcribe to such contours (lower quartile 37 → 12 symbols).
 *  At 20, not one window carrying the right tune was emptied, and noise scored
 *  lower than with no cap at all. 30 starts losing real rank-1 windows. */
export const FF_MIN_QUERY_LENGTH = 20;
