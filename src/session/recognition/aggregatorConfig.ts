...
export interface AggConfig {
  SCORE_FLOOR: number;
  MARGIN_MIN: number;
  /** Minimal top1−top2 margin for a window to count as a win at all (distinct
   *  from MARGIN_MIN, which only kicks in once a tune is already confirmed).
   *  0 = disabled, matching every window ever tested before this field
   *  existed. See aggregator.ts's step(). */
  MARGIN_FLOOR: number;
  K_CONFIRM: number;
  K_SWITCH: number;
...
export const AGG_CONFIG: AggConfig = {
  ...
  MARGIN_MIN: 0.04,
  /** Disabled by default (0) — see the interface doc. Under evaluation
   *  2026-08-11: talking-not-playing windows were observed scoring HIGH
   *  (0.49-0.78, well above SCORE_FLOOR) with an exact top1==top2 tie every
   *  time across a real 30 s stretch, while a real but quietly/poorly
   *  transcribed tune (Dancing Eyes, never above 0.34) had a window with a
   *  genuine 17.8% margin — margin looks like a better "is there a real
   *  match" signal than absolute score alone, at least in that one test. */
  MARGIN_FLOOR: 0,
